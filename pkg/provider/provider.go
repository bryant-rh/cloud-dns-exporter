...
func init() {
	...
	Factory.Register(public.AliyunDnsProvider, func(account map[string]string) DNSProvider {
		// 处理EnablePrivateDNS字段，默认为false
		enablePrivateDNS := false
		if val, exists := account["enablePrivateDNS"]; exists {
			enablePrivateDNS = strings.ToLower(val) == "true"
		}
		...
		return &AliyunDNS{
			account: public.Account{
				...
				SecretID:         account["secretId"],
				SecretKey:        account["secretKey"],
				RoleArn:          account["roleArn"], // 新增：STS ARN 支持
				EnablePrivateDNS: enablePrivateDNS,   // 新增：内网域名监控开关
			},
		}
	})
...
type Domain struct {
	...
	DomainType      string `json:"domain_type"` // 新增：域名类型 public/private
